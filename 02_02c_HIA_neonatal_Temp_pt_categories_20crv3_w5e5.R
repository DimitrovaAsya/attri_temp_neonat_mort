...
######                                                                     ######
######      CODE: HIA [CATEGORIES] - Neonatal Deaths; [20crv3-w5e5]        ######
################################################################################# 
library(reshape2)
library(here)
library(survival)
...
attr_frac_df <- df_0 %>% 
  mutate(scen="obsclim",
         sim="20crv3-w5e5",
         range ="extremely hot") %>% 
  write.csv(file= here("Results", "Attributable_neonatal", "20crv3-w5e5", 
...
for(j in countries){
  ...
  set.seed(42)
  CIs_hot <- quantile(attrdl(Qneonat,cb.temp,df_2$neonat_mort, model, 
                             type="af", cen=cen, range=c(75, 97.5), sim=TRUE,
                             nsim=10000),c(2.5,97.5)/100, na.rm=TRUE)  
  CIs_hot <- as.data.frame(t(CIs_hot))
  CIs_hot <- CIs_hot %>% 
    dplyr::rename('attr_f_hl' = '2.5%',
...
for(j in countries){
  ...
  CIs_cold <- quantile(attrdl(Qneonat,cb.temp,df_2$neonat_mort, model, 
                              type="af", cen=cen,range=c(25, cen), sim=TRUE,
                              nsim=10000),c(2.5,97.5)/100, na.rm=TRUE) 
...
df_1 <- df_1 %>% 
  write.csv(here("Results", "Attributable_neonatal", "Monte_Carlo",
                 "Factual_20crv3-w5e5_extr_cold.csv"))
...
extr_h <- read.csv(here("Results", "Attributable_neonatal", "20crv3-w5e5", 
                        "obsclim_20crv3-w5e5_extr_hot.csv")) %>% 
  dplyr::select(c("country", "attr_f_hot","range", "scen", "sim"))

mod_h <- read.csv(here("Results", "Attributable_neonatal", "20crv3-w5e5", 
                       "obsclim_20crv3-w5e5_mod_hot.csv")) %>% 
  dplyr::select(c("country", "attr_f_hot","range", "scen", "sim"))

mild_h <- read.csv(here("Results", "Attributable_neonatal", "20crv3-w5e5", 
...
extr_c <- read.csv(here("Results", "Attributable_neonatal", "20crv3-w5e5", 
                        "obsclim_20crv3-w5e5_extr_cold.csv")) %>% 
  dplyr::select(c("country", "attr_f_cold","range", "scen", "sim"))

mod_c <- read.csv(here("Results", "Attributable_neonatal", "20crv3-w5e5", 
                       "obsclim_20crv3-w5e5_mod_cold.csv")) %>% 
  dplyr::select(c("country", "attr_f_cold","range", "scen", "sim"))

mild_c <- read.csv(here("Results", "Attributable_neonatal", "20crv3-w5e5", 
                        "obsclim_20crv3-w5e5_mild_cold.csv")) %>% 
  dplyr::select(c("country", "attr_f_cold","range", "scen", "sim"))
...
jpeg(here("Figures", "Neonatal mortality" , "Stage II", "20crv3-w5e5", 
          "heat_cold_rate_20crv3-w5e5_6categories.jpg"), units = 'in', 
           res = 300, width = 4, height = 6)

ggplot(attr_frac_1, aes(x=country, y=value, fill=range)) +
  geom_bar(position = "stack", stat="identity", color="black")+  
  scale_fill_manual(values=c('#e85b54', '#eb7c60', '#eba160','#9bd2f2',  
                             '#4193f0', '#415ef0'))+

...
#######                                                                    ######
#######   CODE: HIA - Very Early Neonatal Deaths [20crv3-w5e5]             ######
################################################################################# 
#################################################################################  
...
attr_frac_df <- df_0 %>% 
  mutate(scen="obsclim",
         sim="20crv3-w5e5") %>% 
  write.csv(file= here("Results", "Attributable_ve_neonats", "20crv3-w5e5", "obsclim_20crv3-w5e5.csv"),
            row.names=FALSE)
write.csv(df_1, here("Results", "Attributable_ve_neonats", "Monte_Carlo", "Factual_20crv3-w5e5.csv"))
...
write.csv(sim, here("Results", "Attributable_ve_neonats", "Monte_Carlo", 
                    "Factual_20crv3-w5e5_all_countries.csv"))
...
for(j in countries){
    ...
    set.seed(42)
    CIs_hot <- quantile(attrdl(Qve_neonat,cb.temp,df_2$ve_neonat, model, 
                         type="af", cen=cen,range=c(cen, 100), sim=TRUE,
...
attr_frac_df <- df_0 %>% 
  ...
  write.csv(file= here("Results", "Attributable_ve_neonats", "20crv3-w5e5", 
                       "counterclim_20crv3-w5e5.csv"), row.names=FALSE)

write.csv(df_1, here("Results", "Attributable_ve_neonats", "Monte_Carlo", 
                     "Counterfactual_20crv3-w5e5.csv"))
...
sim <- cbind(sim_hot,sim_cold) 
write.csv(sim, here("Results", "Attributable_ve_neonats", "Monte_Carlo", 
                    "Counterfactual_20crv3-w5e5_all_countries.csv"))

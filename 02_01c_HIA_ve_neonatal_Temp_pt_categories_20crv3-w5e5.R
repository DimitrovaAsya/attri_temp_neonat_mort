...
######                                                                     ######
###### CODE: HIA [CATEGORIES] - Very Early Neonatal Deaths; [20crv3-w5e5]  ######
################################################################################# 
#################################################################################  
...
for(j in countries){
  ...
  # ATTRIBUTABLE FRACTION COMPONENT DUE TO HEAT AND COLD - 10000 MC simulations
  #95% CIs calculated
  set.seed(42)
...
attr_frac_df <- df_0 %>% 
  mutate(scen="obsclim",
         sim="20crv3-w5e5",
         range ="extremely hot") %>% 
  write.csv(file= here("Results", "Attributable_ve_neonats", "20crv3-w5e5", 
            "obsclim_20crv3-w5e5_extr_hot.csv"), row.names=FALSE)

df_1 <- df_1 %>% 
  mutate(range ="extremely hot") %>% 
  write.csv(file= here("Results", "Attributable_ve_neonats", "Monte_Carlo", 
                       "Factual_20crv3-w5e5_extr_hot.csv"))
...
data <- data %>% 
  dplyr::select(c(ve_neonat, clim.zone, strata_id, CountryName, year, 
                  tmean_pct_lag0, tmean_pct_lag1, tmean_pct_lag2)) 
...
attr_frac_df <- df_0 %>% 
  ...
  write.csv(file= here("Results", "Attributable_ve_neonats", "20crv3-w5e5", 
            "obsclim_20crv3-w5e5_mod_hot.csv"), row.names=FALSE)


df_1 <- df_1 %>% 
  mutate(range ="moderately hot") %>% 
  write.csv(file= here("Results", "Attributable_ve_neonats", "Monte_Carlo", 
                       "Factual_20crv3-w5e5_mod_hot.csv"))
...
attr_frac_df <- df_0 %>% 
  mutate(scen="obsclim",
         ...
         range ="mildly hot") %>% 
  write.csv(file= here("Results", "Attributable_ve_neonats", "20crv3-w5e5", 
      "obsclim_20crv3-w5e5_mild_hot.csv"), row.names=FALSE)

df_1 <- df_1 %>% 
  mutate(range ="mildly hot") %>% 
  write.csv(here("Results", "Attributable_ve_neonats", "Monte_Carlo", 
                 "Factual_20crv3-w5e5_mild_hot.csv"))
...
for(j in countries){
  ...
  attr_f_cold <- attrdl(Qve_neonat, cb.temp,df_2$ve_neonat, model, type="af", cen=cen,
                        range=c(25, cen))  
  ...
  sim_cold$country <- j
  sim_cold$scenario <- "Factual"
  sim_cold$dataset <- "20crv3-w5e5"
...
attr_frac_df <- df_0 %>% 
  mutate(scen="obsclim",
         ...
         range ="mildly cold") %>% 
  write.csv(file= here("Results", "Attributable_ve_neonats", "20crv3-w5e5", 
        "obsclim_20crv3-w5e5_mild_cold.csv"), row.names=FALSE)

df_1 <- df_1 %>% 
  mutate(range ="mildly cold") %>% 
  write.csv(here("Results", "Attributable_ve_neonats", "Monte_Carlo", 
                 "Factual_20crv3-w5e5_mild_cold.csv"))
...
for(j in countries){
  ...
  set.seed(42)
  CIs_cold <- quantile(attrdl(Qve_neonat,cb.temp,df_2$ve_neonat, model, 
             type="af", cen=cen, range=c(2.5, 25), sim=TRUE, 
...
attr_frac_df <- df_0 %>% 
  ...
  write.csv(file= here("Results", "Attributable_ve_neonats", "20crv3-w5e5",
          "obsclim_20crv3-w5e5_mod_cold.csv"), row.names=FALSE)


df_1 <- df_1 %>% 
  write.csv(here("Results", "Attributable_ve_neonats", "Monte_Carlo",
                 "Factual_20crv3-w5e5_mod_cold.csv"))
...
attr_frac_df <- df_0 %>% 
  ...
  write.csv(file= here("Results", "Attributable_ve_neonats", "20crv3-w5e5", 
          "obsclim_20crv3-w5e5_extr_cold.csv"), row.names=FALSE)

df_1 <- df_1 %>% 
  write.csv(here("Results", "Attributable_ve_neonats", "Monte_Carlo", "Factual_20crv3-w5e5_extr_cold.csv"))
...
extr_h <- read.csv(here("Results", "Attributable_ve_neonats", "20crv3-w5e5", 
                        "obsclim_20crv3-w5e5_extr_hot.csv")) %>% 
  dplyr::select(c("country", "attr_f_hot","range", "scen", "sim"))

mod_h <- read.csv(here("Results", "Attributable_ve_neonats", "20crv3-w5e5", 
                       "obsclim_20crv3-w5e5_mod_hot.csv")) %>% 
  dplyr::select(c("country", "attr_f_hot","range", "scen", "sim"))

mild_h <- read.csv(here("Results", "Attributable_ve_neonats", "20crv3-w5e5", 
...
extr_c <- read.csv(here("Results", "Attributable_ve_neonats", "20crv3-w5e5", 
                        "obsclim_20crv3-w5e5_extr_cold.csv")) %>% 
  dplyr::select(c("country", "attr_f_cold","range", "scen", "sim"))

mod_c <- read.csv(here("Results", "Attributable_ve_neonats", "20crv3-w5e5", 
                       "obsclim_20crv3-w5e5_mod_cold.csv")) %>% 
  dplyr::select(c("country", "attr_f_cold","range", "scen", "sim"))

mild_c <- read.csv(here("Results", "Attributable_ve_neonats", "20crv3-w5e5", 
                        "obsclim_20crv3-w5e5_mild_cold.csv")) %>% 
  dplyr::select(c("country", "attr_f_cold","range", "scen", "sim"))
...
attr_frac_1 <- attr_frac_1 %>% 
  mutate(range= as.factor(range),
         range=factor(range, c("extremely hot", "moderately hot", 
            "mildly hot", "mildly cold", "moderately cold", "extremely cold"))) 

attr_frac_1 <- left_join(attr_frac_1, continent) 

write.csv(attr_frac_1, here("Results", "Attributable_ve_neonats", "20crv3-w5e5", 
                            "obsclim_20crv3-w5e5_attr_range.csv"))
#Plot attributable fraction by country
jpeg(here("Figures", "ve_neonats" , "Stage II", "20crv3-w5e5", 
          "heat_cold_rate_20crv3-w5e5_6categories.jpg"),

...
#####                                                                      ######
##### CODE: HIA [CATEGORIES] - Very Early Neonatal Deaths; [20crv3-era5]   ######
################################################################################# 
#################################################################################  
...
attr_frac_df <- df_0 %>% 
  ...
  write.csv(file= here("Results", "Attributable_ve_neonats", "20crv3-era5", 
                       "obsclim_20crv3-era5_extr_hot.csv"), row.names=FALSE)
...
df_1 <- df_1 %>% 
  ...
  write.csv(file= here("Results", "Attributable_ve_neonats", "Monte_Carlo", 
                       "Factual_20crv3-era5_extr_hot.csv"))
...
attr_frac_df <- df_0 %>% 
  ...
  write.csv(file= here("Results", "Attributable_ve_neonats", "20crv3-era5", 
                   "obsclim_20crv3-era5_mod_hot.csv"), row.names=FALSE)


df_1 <- df_1 %>% 
  mutate(range ="moderately hot") %>% 
  write.csv(file= here("Results", "Attributable_ve_neonats", "Monte_Carlo", 
                       "Factual_20crv3-era5_mod_hot.csv"))
...
df_1 <- df_1 %>% 
  ...
  write.csv(here("Results", "Attributable_ve_neonats", "Monte_Carlo", 
                 "Factual_20crv3-era5_mild_hot.csv"))
...
attr_frac_df <- df_0 %>% 
  mutate(scen="obsclim",
         ...
         range ="mildly cold") %>% 
  write.csv(file= here("Results", "Attributable_ve_neonats", "20crv3-era5", 
            "obsclim_20crv3-era5_mild_cold.csv"), row.names=FALSE)


df_1 <- df_1 %>% 
  mutate(range ="mildly cold") %>% 
  write.csv(here("Results", "Attributable_ve_neonats", "Monte_Carlo", 
                 "Factual_20crv3-era5_mild_cold.csv"))
...
attr_frac_df <- df_0 %>% 
  ...
  write.csv(file= here("Results", "Attributable_ve_neonats", "20crv3-era5", 
             "obsclim_20crv3-era5_mod_cold.csv"), row.names=FALSE)

df_1 <- df_1 %>% 
  write.csv(here("Results", "Attributable_ve_neonats", "Monte_Carlo", 
                 "Factual_20crv3-era5_mod_cold.csv"))
...
for(j in countries){
  ...
  set.seed(42)
  CIs_cold <- quantile(attrdl(Qve_neonat,cb.temp,df_2$ve_neonat, model, 
      type="af", cen=cen,range=c(0, 2.5), sim=TRUE,nsim=10000), c(2.5,97.5)/100, 
      na.rm=TRUE) 
...
attr_frac_df <- df_0 %>% 
  ...
  write.csv(file= here("Results", "Attributable_ve_neonats", "20crv3-era5", 
            "obsclim_20crv3-era5_extr_cold.csv"), row.names=FALSE)


df_1 <- df_1 %>% 
  write.csv(here("Results", "Attributable_ve_neonats", "Monte_Carlo", 
                 "Factual_20crv3-era5_extr_cold.csv"))
...
extr_h <- read.csv(here("Results", "Attributable_ve_neonats", "20crv3-era5", 
      "obsclim_20crv3-era5_extr_hot.csv")) %>% 
       dplyr::select(c("country", "attr_f_hot","range", "scen", "sim"))

mod_h <- read.csv(here("Results", "Attributable_ve_neonats", "20crv3-era5", 
                       "obsclim_20crv3-era5_mod_hot.csv")) %>% 
       dplyr::select(c("country", "attr_f_hot","range", "scen", "sim"))

mild_h <- read.csv(here("Results", "Attributable_ve_neonats", "20crv3-era5", 
                        "obsclim_20crv3-era5_mild_hot.csv")) %>% 
       dplyr::select(c("country", "attr_f_hot","range", "scen", "sim"))
...
extr_c <- read.csv(here("Results", "Attributable_ve_neonats", "20crv3-era5", 
                        "obsclim_20crv3-era5_extr_cold.csv")) %>% 
  dplyr::select(c("country", "attr_f_cold","range", "scen", "sim"))

mod_c <- read.csv(here("Results", "Attributable_ve_neonats", "20crv3-era5", 
                       "obsclim_20crv3-era5_mod_cold.csv")) %>% 
  dplyr::select(c("country", "attr_f_cold","range", "scen", "sim"))

mild_c <- read.csv(here("Results", "Attributable_ve_neonats", "20crv3-era5", 
                        "obsclim_20crv3-era5_mild_cold.csv")) %>% 
   dplyr::select(c("country", "attr_f_cold","range", "scen", "sim"))
...
write.csv(attr_frac_1, here("Results", "Attributable_ve_neonats", "20crv3-era5", 
                            "obsclim_20crv3-era5_attr_range.csv"))
#Plot attributable fraction by country
jpeg(here("Figures", "ve_neonats" , "Stage II", "20crv3-era5", "heat_cold_rate_20crv3-era5_6categories.jpg"),

...
#######                                                                    ######
#######     CODE: HIA [CATEGORIES] - Neonatal Deaths; [20crv3-era5]        ######
################################################################################# 
#################################################################################  
library(reshape2)
library(here)
...
for(j in countries){
  ...
  CIs_hot <- quantile(attrdl(Qneonat,cb.temp,df_2$neonat_mort, model, 
                            type="af", cen=cen, range=c(97.5,100), 
...
attr_frac_df <- df_0 %>% 
  mutate(scen="obsclim",
         sim="20crv3-era5",
         range ="extremely hot") %>% 
  write.csv(file= here("Results", "Attributable_neonatal", "20crv3-era5", 
...
attr_frac_df <- df_0 %>% 
  mutate(scen="obsclim",
         ...
         range ="mildly hot") %>% 
  write.csv(file= here("Results", "Attributable_neonatal", "20crv3-era5", 
              "obsclim_20crv3-era5_mild_hot.csv"), row.names=FALSE)
...
extr_h <- read.csv(here("Results", "Attributable_neonatal", "20crv3-era5", 
                        "obsclim_20crv3-era5_extr_hot.csv")) %>% 
  dplyr::select(c("country", "attr_f_hot","range", "scen", "sim"))

mod_h <- read.csv(here("Results", "Attributable_neonatal", "20crv3-era5", 
                       "obsclim_20crv3-era5_mod_hot.csv")) %>% 
  dplyr::select(c("country", "attr_f_hot","range", "scen", "sim"))

mild_h <- read.csv(here("Results", "Attributable_neonatal", "20crv3-era5", 
...
extr_c <- read.csv(here("Results", "Attributable_neonatal", "20crv3-era5", 
                        "obsclim_20crv3-era5_extr_cold.csv")) %>% 
  dplyr::select(c("country", "attr_f_cold","range", "scen", "sim"))

mod_c <- read.csv(here("Results", "Attributable_neonatal", "20crv3-era5", 
                       "obsclim_20crv3-era5_mod_cold.csv")) %>% 
  dplyr::select(c("country", "attr_f_cold","range", "scen", "sim"))

mild_c <- read.csv(here("Results", "Attributable_neonatal", "20crv3-era5", 
                        "obsclim_20crv3-era5_mild_cold.csv")) %>% 
  dplyr::select(c("country", "attr_f_cold","range", "scen", "sim"))
...
jpeg(here("Figures", "Neonatal mortality" , "Stage II", "20crv3-era5", "heat_cold_rate_20crv3-era5_6categories.jpg"),

...
#######                                                                    ######
#######           CODE: HIA - Neonatal Deaths [20crv3-w5e5]                ######
################################################################################# 
#################################################################################  
...
attr_frac_df <- df_0 %>% 
  mutate(scen="obsclim",
   ...
   write.csv(file= here("Results", "Attributable_neonatal", "20crv3-w5e5", 
                  "obsclim_20crv3-w5e5_0-2.csv"), row.names=FALSE)

 write.csv(df_1, here("Results", "Attributable_neonatal", "Monte_Carlo", 
                      "Factual_20crv3-w5e5.csv"))
 ...
 sim_cold <- attrdl(Qneonat ,cb.temp,data$neonat_mort, model, 
                    type="af",cen=cen, range=c(-100,cen), sim=TRUE,nsim=10000)
 sim_cold <- as.data.frame(sim_cold)

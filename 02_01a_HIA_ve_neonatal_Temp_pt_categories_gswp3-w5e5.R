...
#######                                                                    ######
####### CODE: HIA [CATEGORIES] - Very Early Neonatal Deaths; [gswp3-w5e5]   #####
################################################################################# 
#################################################################################  
...
for(j in countries){
  ...
  CIs_hot <- quantile(attrdl(Qneonat,cb.temp,df_2$ve_neonat, model, 
    type="af", cen=cen, range=c(97.5,100), sim=TRUE,nsim=10000),c(2.5,97.5)/100, na.rm=TRUE)  
...
attr_frac_df <- df_0 %>% 
  mutate(scen="obsclim",
         sim="gswp3-w5e5",
         range ="extremely hot") %>% 
  write.csv(file= here("Results", "Attributable_ve_neonats", "gswp3-w5e5", "obsclim_gswp3-w5e5_extr_hot.csv"),
            row.names=FALSE)


df_1 <- df_1 %>% 
  mutate(range ="extremely hot") %>% 
  write.csv(file= here("Results", "Attributable_ve_neonats", "Monte_Carlo", "Factual_gswp3-w5e5_extr_hot.csv"))
...
for(j in countries){
  ...
  set.seed(42)
  CIs_hot <- quantile(attrdl(Qneonat,cb.temp,df_2$ve_neonat, model, 
        type="af", cen=cen, range=c(75, 97.5), sim=TRUE,nsim=10000),c(2.5,97.5)/100, na.rm=TRUE)  
  CIs_hot <- as.data.frame(t(CIs_hot))
  CIs_hot <- CIs_hot %>% 
    dplyr::rename('attr_f_hl' = '2.5%',
                  'attr_f_hh' = '97.5%') 
...
attr_frac_df <- df_0 %>% 
  ...
  write.csv(file= here("Results", "Attributable_ve_neonats", "gswp3-w5e5", "obsclim_gswp3-w5e5_mod_hot.csv"),
            row.names=FALSE)

df_1 <- df_1 %>% 
  mutate(range ="moderately hot") %>% 
  write.csv(file= here("Results", "Attributable_ve_neonats", "Monte_Carlo", "Factual_gswp3-w5e5_mod_hot.csv"))
...
for(j in countries){
  ...
  set.seed(42)
  CIs_hot <- quantile(attrdl(Qneonat,cb.temp,df_2$ve_neonat, model, 
           type="af", cen=cen, range=c(cen ,75), sim=TRUE,nsim=10000),c(2.5,97.5)/100, na.rm=TRUE)  
  CIs_hot <- as.data.frame(t(CIs_hot))
  CIs_hot <- CIs_hot %>% 
    dplyr::rename('attr_f_hl' = '2.5%',
                  'attr_f_hh' = '97.5%') 
...
attr_frac_df <- df_0 %>% 
  mutate(scen="obsclim",
         sim="gswp3-w5e5",
         range ="mildly hot") %>% 
  write.csv(file= here("Results", "Attributable_ve_neonats", "gswp3-w5e5", "obsclim_gswp3-w5e5_mild_hot.csv"),
            row.names=FALSE)

df_1 <- df_1 %>% 
  mutate(range ="mildly hot") %>% 
write.csv(here("Results", "Attributable_ve_neonats", "Monte_Carlo", "Factual_gswp3-w5e5_mild_hot.csv"))
...
for(j in countries){
  ...
  CIs_cold <- quantile(attrdl(Qneonat,cb.temp,df_2$ve_neonat, model, 
        type="af", cen=cen,range=c(25, cen), sim=TRUE,nsim=10000),c(2.5,97.5)/100, na.rm=TRUE) 
...
attr_frac_df <- df_0 %>% 
  mutate(scen="obsclim",
         sim="gswp3-w5e5",
         range ="mildly cold") %>% 
  write.csv(file= here("Results", "Attributable_ve_neonats", "gswp3-w5e5", "obsclim_gswp3-w5e5_mild_cold.csv"),
            row.names=FALSE)

df_1 <- df_1 %>% 
  mutate(range ="mildly cold") %>% 
  write.csv(here("Results", "Attributable_ve_neonats", "Monte_Carlo", "Factual_gswp3-w5e5_mild_cold.csv"))
...
for(j in countries){
  ...
  set.seed(42)
  CIs_cold <- quantile(attrdl(Qneonat,cb.temp,df_2$ve_neonat, model, 
     type="af", cen=cen,range=c(2.5, 25), sim=TRUE,nsim=10000),c(2.5,97.5)/100, na.rm=TRUE) 
...
attr_frac_df <- df_0 %>% 
  ...
  write.csv(file= here("Results", "Attributable_ve_neonats", "gswp3-w5e5", "obsclim_gswp3-w5e5_mod_cold.csv"),
            row.names=FALSE)

df_1 <- df_1 %>% 
   write.csv(here("Results", "Attributable_ve_neonats", "Monte_Carlo", "Factual_gswp3-w5e5_mod_cold.csv"))
...
for(j in countries){
  ...
  set.seed(42)
  CIs_cold <- quantile(attrdl(Qneonat,cb.temp,df_2$ve_neonat, model, 
      type="af", cen=cen,range=c(0, 2.5), sim=TRUE,nsim=10000),c(2.5,97.5)/100, na.rm=TRUE) 
...
attr_frac_df <- df_0 %>% 
  ...
  write.csv(file= here("Results", "Attributable_ve_neonats", "gswp3-w5e5", "obsclim_gswp3-w5e5_extr_cold.csv"),
            row.names=FALSE)

df_1 <- df_1 %>% 
   write.csv(here("Results", "Attributable_ve_neonats", "Monte_Carlo", "Factual_gswp3-w5e5_extr_cold.csv"))
...
extr_h <- read.csv(here("Results", "Attributable_ve_neonats", "gswp3-w5e5", "obsclim_gswp3-w5e5_extr_hot.csv")) %>% 
  dplyr::select(c("country", "attr_f_hot","range", "scen", "sim"))

mod_h <- read.csv(here("Results", "Attributable_ve_neonats", "gswp3-w5e5", "obsclim_gswp3-w5e5_mod_hot.csv")) %>% 
  dplyr::select(c("country", "attr_f_hot","range", "scen", "sim"))

mild_h <- read.csv(here("Results", "Attributable_ve_neonats", "gswp3-w5e5", "obsclim_gswp3-w5e5_mild_hot.csv")) %>% 
  dplyr::select(c("country", "attr_f_hot","range", "scen", "sim"))
...
extr_c <- read.csv(here("Results", "Attributable_ve_neonats", "gswp3-w5e5", "obsclim_gswp3-w5e5_extr_cold.csv")) %>% 
  dplyr::select(c("country", "attr_f_cold","range", "scen", "sim"))

mod_c <- read.csv(here("Results", "Attributable_ve_neonats", "gswp3-w5e5", "obsclim_gswp3-w5e5_mod_cold.csv")) %>% 
  dplyr::select(c("country", "attr_f_cold","range", "scen", "sim"))

mild_c <- read.csv(here("Results", "Attributable_ve_neonats", "gswp3-w5e5", "obsclim_gswp3-w5e5_mild_cold.csv")) %>% 
  dplyr::select(c("country", "attr_f_cold","range", "scen", "sim"))
...
ve_neonats_data <- read_excel(here("Data/ve_neonats_data_unicef.xlsx")) 
...
#Calculate ve_neonats by country for the whole period (2001-2019)
ve_neonats_data <- ve_neonats_data %>% 
  dplyr::select(country, Year, total_ve_neonats) %>% 
  dplyr::mutate(total_ve_neonats= as.numeric(total_ve_neonats)) %>% 
  group_by(country) %>% 
  summarise(total_ve_neonats = sum(total_ve_neonats))


total_attr <- bind_rows(hot_attr,cold_attr) %>% 
  left_join(ve_neonats_data) %>%
  left_join(births_data) %>% 
  mutate(attr_f_total = attr_f_total*total_ve_neonats,
         # total_upp  = sim_hot_upp*total_ve_neonats,
         # total_low  = sim_hot_low*total_ve_neonats,
...
write.csv(attr_frac_1, here("Results", "Attributable_ve_neonats", "gswp3-w5e5", "obsclim_gswp3-w5e5_attr_range.csv"))
#Plot attributable fraction by country
jpeg(here("Figures", "ve_neonats" , "Stage II", "gswp3-w5e5", "heat_cold_rate_gswp3-w5e5_6categories.jpg"),
...
ggplot(attr_frac_1, aes(x=country, y=value, fill=range)) +
  ...
  labs(x ="Country", y = "Heat- and cold-related ve_neonat rate (per 100,000)")+  
  theme_minimal()+
